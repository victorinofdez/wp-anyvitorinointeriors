...
// Codigo para obtener los datos para mostrar los resultados
    ...
    // hayamos los parametros de busqueda para ese link
    if ( is_category() ) {
        $words = single_cat_title('', false).' category'; // devuelve la categoria que estas manejando en el momento,
                                  //el primero se deja cadena vacia para que no salga categoría el segundo parametro indica que no te lo enseñe sino que te lo devuelva
    } elseif ( is_tag() ) {
        $words = single_tag_title('', false).' tag'; // igual que lo anterior pero para las tags
    } elseif ( is_date() ) {
        $words = get_the_date('j F, Y');
    } elseif ( is_month() ) {
        $words = get_the_date('F, Y');
    } elseif ( is_year() ) {
        $words = get_the_date('Y');
    } elseif ( is_author() ) {
        // Leemos el primer registros para extrar el autor, pero el puntero avanza una posición
        the_post();
        $words = get_the_author();// obtenemos el autor
        
        // Rebobinamos el puntero del result sets de los posts
        rewind_posts();
    }
					...
					************************************** -->
                			...
                			<div class="post-list-search">
											...
											if (have_posts()):
												while (have_posts()):
													...
													if (has_post_thumbnail()) {
														...
														if (get_post_type() == 'page') { // es una pagina
															// ponemos una imagen por defecto para cada página
															switch ($post->post_title) {
																case 'HOME':
																	$thumb_url = get_template_directory_uri() . '/assets/images/shop/slider1.jpg';
																	break;
																case 'BLOG':
																	$thumb_url = get_template_directory_uri() . '/assets/images/shop/slider1.jpg';
																	break;
															...
															$thumb_url = get_template_directory_uri() . '/assets/img/logo-light.png';
														}
													}
										?>
													<li>
										...
										<?php
												...
												echo 'No posts found for: ' . get_search_query();
											endif;
										?>
												</ul>
											</div>
